...
4. Run: bash "${DIR}/scripts/state.sh"  (branch, last task, tree, tests in one shot)
...
5. Read ${DIR}/docs/CODEBASE-KNOWLEDGE.md (file map, conventions, TRAPS - do not rediscover or "fix" deliberate design) and ${DIR}/.claude/ZONING.md (Article X as literal first-match-wins triggers - run it on every change you plan).

ENFORCEMENT RAILS (mechanical - they will block you, that is by design):
- bash scripts/verify-gates.sh MUST print ALL GATES PASS before your commit (git pre-commit re-runs it). Checklists: writes in src/db -> .claude/checklists/per-write.md; report code -> .claude/checklists/per-report.md.
- Claude hooks + git hooks block: git add -A, --no-verify, writes to specs/.specify/schema.sql/gates, LLM tokens, fail-open secrets, Phase-B tokens, missing commit trailer. If a rail blocks you and seems WRONG, return status=blocked explaining why - never adjust a rail, never route around it.

ANTI-HANG RULES (a prior T009 attempt died here - non-negotiable):
- Every test that starts a server MUST stop it: use port 0, keep the Server handle, call server.stop(true) in afterEach/afterAll, and close every Database handle. If "bun test" does not exit promptly, the leaked handle IS the bug - fix it before anything else (systematic-debugging skill).
- NEVER start a long-lived foreground process (bun run dev, a server without stop). To smoke-test the dev entry point, start it in the background, curl it, then kill it - in one command with a bounded timeout.
- Give every potentially-slow Bash call an explicit timeout; if a command might block, wrap it (e.g. "timeout 60 bun test" via gtimeout/timeout if available, or split the verification into smaller bounded calls).
...
- Buyers here are ANALYTICAL ONLY - reject any name/phone/email keys at runtime (Article IV).` },
  ...
  { id: 'T009', role: 'You are a Bun.serve HTTP API specialist: routing, boundary validation, ephemeral-port test servers.',
    brief: `Test-first tests/api.test.ts then src/api/server.ts: POST /leads /viewings /offers, GET /pipeline?project= and /counters?project=. Thin boundary - validate shape/types then delegate to queries.ts; NO business logic. Empty next_action -> 400 GREEK JSON error; viewing without interest -> 400 (required); consistent 400/404 semantics. Export makeServer(db) so tests inject a temp DB and port 0; import.meta.main (and ONLY it) starts the dev server. Tests drive real HTTP via fetch: happy path per endpoint + Article II rejection per write + viewing-without-interest.
CRITICAL TEARDOWN (the prior attempt at this task died on a hung "bun test" - a leaked Bun.serve handle keeps the process alive): structure tests with beforeEach creating {db, server} and afterEach calling server.stop(true) AND db.close(). After writing tests, run "bun test tests/api.test.ts" and confirm the process EXITS (prompt returns) - if it hangs, the leaked handle is your bug, fix it first. Never run "bun run dev" in the foreground.` },
  ...
  { id: 'T010a', role: 'You are the data-layer specialist: atomic transactions.',
    brief: `RULING CONTEXT (a previous attempt correctly halted RED on Gate 6c): the human-side ruling (RULING 2026-07-13b in DECISIONS.md) scoped Gate 6c to capture writes, and your predecessor's finished work was committed as T010a (updateAskingPrice + 7 tests + ADR-0019). Verify that commit satisfies this brief and return status=done with its SHA - do not rebuild. Original brief: updateAskingPrice(unitId, newPrice, reason) in queries.ts: update units.asking_current AND append exactly one price_changes row (old/new/reason/changed_at ISO) in ONE transaction - both or neither. Test-first: exactly one log row with correct old/new per call; reads see the new price; rollback path if forceable.` },

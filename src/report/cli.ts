...
const USAGE = `Χρήση: bun run report --builder="Κατασκευαστική Άλφα ΑΕ" --project="Ρετιρέ Κύπρου" \\
       --period=biweekly|monthly|quarterly [--anchor=YYYY-MM-DD] [--rolling] [--as-of=YYYY-MM-DD] [--brief] [--html]
   ή:  bun run report --separation [--html]
...
Παράγει την αναφορά προόδου πωλήσεων του έργου σε Markdown (ελληνικά) στο stdout.
  ...
  --html       η ίδια αναφορά ως αυτοτελές έγγραφο HTML (για αποστολή στον
               κατασκευαστή με email/Viber)· συνδυάζεται με κάθε περίοδο και με
               το --separation, όχι όμως με το --brief
  --separation αυτόνομη αναφορά ελέγχου διαχωρισμού (SC-5): κατανομή γεγονότων
               ανά χειριστή από το v_separation, για όλα τα έργα — δεν απαιτεί
               άλλες παραμέτρους και υπερισχύει όσων δοθούν
...
if (import.meta.main) {
  ...
  try {
    parsed = parseArgs({
      ...
      options: {
        ...
        separation: { type: "boolean", default: false },
  ...
  if (v.html === true && v.brief === true) {
    // B0b — the HTML document is the builder-facing deliverable; the brief is
    // the internal /insights JSON. One stdout cannot be both.
    console.error(
      "Σφάλμα αναφοράς: οι παράμετροι --html και --brief δεν συνδυάζονται — το --html αφορά την αναφορά προς τον κατασκευαστή, το --brief το εσωτερικό βήμα /insights.",
    );
    process.exit(1);
  }
  if (v.separation === true) {
    // T019 — SC-5 standalone separation report: global over ALL projects, so it
    // needs no --builder/--project/--period. Branching BEFORE the required-flag
    // validation is what gives it the documented precedence over report flags.
    // separationReport is total over data (renders a complete page on an empty
    // DB), so no Greek-error catch is needed on this path.
    const db = initDb();
    try {
      const page = separationReport(db);
      process.stdout.write(v.html === true ? htmlDocument(page) : page);
    } finally {
      db.close();
    }
    process.exit(0);
